...
function populatePermanentUpgrades() {
  ...
  for (const key in permanentUpgrades) {
    ...
    upgradeEl.className =
      'bg-gray-700 p-4 rounded-lg flex flex-col justify-between border border-gray-600';
    let buttonHTML = '';
  ...
  dom.upgradesContainerEl
    .querySelectorAll('.buy-upgrade-btn')
    .forEach((btn) => btn.addEventListener('click', handleBuyPermanentUpgrade));
}
...
function handleItemSelect(item, itemEl) {
  ...
  if (currentlySelected) {
    ...
    currentlySelected.querySelector('.buy-item-btn').textContent =
      `BUY (${currentlySelected.dataset.itemCost} G)`;
  }
